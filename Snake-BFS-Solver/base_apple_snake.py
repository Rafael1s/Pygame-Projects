...
instead this can be given as follows:
    
from dataclasses import dataclass

@dataclass ##  smth with @ is called decorator
class Book:
    title: str
    author: str
...
class Base:
    cell_size: int = 16 ## 20
    cell_width: int = 14 # 18 ## 12 ## 18 ## 24 ##  12
    cell_height: int = 14 # 18  ##  18 ## 24 ## 12
...
class Apple(Base):
    ...
    def refresh(self, snake):
        ...
        available_positions = set(product(range(self.cell_width), range(self.cell_height))) - set(snake.body)
        ...
        ''' Experiments '''
        ## self.location = list(available_positions)[0] ### full cycle
        list_loc = list(available_positions)
        
        # n = 1,2,3, 4, 36, full cycle
        # n = 5 , cycle = 188
        # n = 6, cycle = 194
        # n = 120, cycle = 189  
        # n = 11. cycle = 190
        ...
        ## n = 36  # cycle = 57 !!
        n = 3
        self.location = self.non_random_location(n, list_loc, snake)
        

    def non_random_location(self, n, list_loc, snake):
        """
        Generate non-random location of a new apple
        n - is the number in availbale position list_loc
        """
        if len(list_loc) > n:
             location = list_loc[n]
        else:  
             ## print('len snake.body: ', len(snake.body), 'available_positions: ', len(list_loc))
             if len(list_loc) >= 1:
                  location = list_loc[0]
             else:    
                  location = (-1, -1)
